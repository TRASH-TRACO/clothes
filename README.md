...
넘어서, 달력을 열 때마다 날짜 수만큼 서버를 부르게 됩니다. 옷장·코디 목록도 카드 수만큼 부릅니다.

그래서 **여러 개가 반복되는 카드·칸 링크는 `prefetch={false}`** 로 둡니다
(`calendar-month.tsx`, `item-card.tsx`, `outfit-card.tsx`). `loading.tsx` 덕분에 눌렀을 때
바로 뼈대가 뜨므로 미리 받아둘 이유가 없습니다. 네비게이션 링크처럼 수가 적고 자주 쓰는 곳은
그대로 미리 받습니다.
